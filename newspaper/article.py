...
class Article:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Markdown version of the article body (generated by parse())
        self.text_markdown = ""

        # Content type: normal | video | podcast | longform | infographic | photo | live
        self.article_type: str = "normal"
    ...
    def parse(self) -> "Article":
        ...
        self.meta_description = self._strip_brand_suffix(
            metadata["description"], metadata.get("site_name", "")
        )
        ...
        from newspaper.outputformatters import build_markdown
        try:
            top_node_html = None
            if self.top_node is not None:
        ...
        self.article_type = self.extractor.get_article_type(self.url, self.doc, self.text)
    ...
    def _strip_brand_suffix(description: str, site_name: str = "") -> str:
        """Remove trailing brand/site-name suffixes from meta descriptions.

        Many sites append their brand after a separator, e.g.:
          "Some article summary. - VnExpress"
          "Some article summary | Tuổi Trẻ Online"
          "Some article summary – Brand"
        """
        import re as _re
        ...
        # Strip Vietnamese newspaper abbreviation prefixes, e.g.:
        #   "(NLĐO)- ", "(TTXVN)- ", "TPO - ", "HNM - "
        _U = r"A-ZĐÁÀẢÃẠĂẮẶẰẲẴÂẤẦẨẪẬÉÈẺẼẸÊẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÚÙỦŨỤƯỨỪỬỮỰÝỲỶỸỴ"
        description = _re.sub(rf"^\([{_U}][{_U}0-9&/]{{1,9}}\)\s*[-–]\s*", "", description)
        description = _re.sub(rf"^[{_U}][{_U}0-9&/]{{1,7}}\s*[-–]\s+", "", description)

        # Separators used between content and brand
        sep_pattern = r"\s*[-–—|]\s*"

        # Build a set of candidate brand tokens to strip:
        # 1. The site_name if known (e.g. "VnExpress", "Tuổi Trẻ Online")
        # 2. A generic heuristic: last token after separator is ≤ 40 chars
        #    and contains no sentence-ending punctuation (i.e. looks like a name)
        if site_name:
            # Escape for regex, allow minor variation
            brand_re = _re.escape(site_name.strip())
            pattern = rf"{sep_pattern}{brand_re}\s*$"
            cleaned = _re.sub(pattern, "", description, flags=_re.IGNORECASE).strip()
            if cleaned != description:
                return cleaned

        # Fallback heuristic: strip " - ShortBrand" if brand is ≤ 40 chars
        # and doesn't look like a sentence fragment (no . ? ! mid-way)
        m = _re.search(rf"{sep_pattern}([^|–—\-]{{1,40}})\s*$", description)
        if m:
            brand = m.group(1).strip()
            # Only strip if it looks like a proper noun / brand name:
            # no lowercase-start words longer than 4 chars (avoid cutting sentences)
            if not _re.search(r"\b[a-záàảãạăắặằẳẵâấầẩẫậéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵđ]{5,}", brand):
